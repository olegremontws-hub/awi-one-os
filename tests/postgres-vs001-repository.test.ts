...
class RecordingDb {
  calls:string[]=[];
  client={
    query:async (sql:string,_params?:unknown[])=>{this.calls.push(sql);return {rows:[],rowCount:1};},
    release:()=>{this.calls.push('RELEASE');},
  };
  async query(sql:string,_params?:unknown[]){this.calls.push(sql);return {rows:[],rowCount:1};}
  async connect(){return this.client as any;}
}

test('PostgreSQL repository commits VS-001 records in one pinned transaction', async () => {
  ...
  const bundle=createVS001PersistenceBundle({
    ...
    analysis:{projectSummary:'Intake',requirements:[],missingInformation:[],risks:[],assumptions:[],confidence:1},
    decision,evidenceRefs:['document:doc-1'],
  });
  await new PostgresVS001Repository(db as SqlClient).commit(bundle);
